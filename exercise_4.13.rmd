...
```{r}
# 1
pick11_1 <- !is.na(johnDoe$data$通報機關名稱)
johnDoe$data$編號[which(pick11_1)]
johnDoe$data[pick11_1, ] -> subsample
View(subsample)
# 2
pick11_2 <- johnDoe$data$通報機關名稱 == "海巡隊"
johnDoe$data$死亡方式描述 [which(pick11_2)]
table(johnDoe$data$死亡方式描述 [which(pick11_2)])
# 3
pick11_3 <- johnDoe$data$死亡方式描述 == "不詳" | johnDoe$data$死亡方式描述 == "他殺"
johnDoe$data$發現縣市 [which(pick11_3)]
table(johnDoe$data$發現縣市 [which(pick11_3)])
```