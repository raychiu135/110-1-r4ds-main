...
```{r}
wdi$data |> remove_nonCountries() -> data_set
data_set$year |> range(na.rm=T) -> year_range
eachYearMean <- vector("numeric", length(year_range[[2]]-year_range[[1]]+1))
i=1
for (.x in year_range[[1]]:year_range[[2]]) {
  data_set |> subset(year==.x) -> data_set.x
  code = "SG.GEN.PARL.ZS"
  data_set.x[[code]] |> mean(na.rm=T) -> eachYearMean[[i]]
  i=i+1
}
i
trendOfMeanIncreasing=TRUE
for (j in 1:(i-2)) {
  if(!is.nan(eachYearMean[[j]])){
    if(eachYearMean[[j]] > eachYearMean[[j+1]]){
      trendOfMeanIncreasing=FALSE
    }
  }
}
year_range
eachYearMean
trendOfMeanIncreasing
```
...
get_meanTrendOverYears <- function(data_set, code){
  data_set$year |> range(na.rm=T) -> year_range
  eachYearMean <- numeric()
  i=1
  for (.x in year_range[[1]]:year_range[[2]]) {
    data_set |> subset(year==.x) -> data_set.x
    code = "SG.GEN.PARL.ZS"
    mean(data_set.x[[code]], na.rm=T) -> eachYearMean[[as.character(.x)]]
    i=i+1
  }
  return(eachYearMean)
}
